...
// Use a safe path for logs, ideally /tmp or the app root
...
const logDebug = (step, data = null) => {
    try {
        ...
        const safeData = data ? JSON.stringify(data, (key, value) => {
            if (key === 'socket' || key === 'client') return '[Circular]';
            return value;
        }) : '';

        const logLine = `[${timestamp}] [UPLOAD DEBUG] ${step} ${safeData}\n`;

        // Console log for stdout (CloudWatch/Docker logs)
        console.log(logLine.trim());

        // File log for persistence
        if (process.env.NODE_ENV !== 'test') { // Optional check
            try {
                fs.appendFileSync(DEBUG_LOG_FILE, logLine);
            } catch (fsError) {
                console.error('Failed to write to log file:', fsError.message);
            }
        }
    } catch (e) {
